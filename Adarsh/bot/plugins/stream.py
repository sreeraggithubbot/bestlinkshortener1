...
async def private_receive_handler(c: Client, m: Message): 
    ...
    try: 
        ...
        stream_link = get_shortlink(online_links) 
        online_link = get_shortlink(online_links)
        tutorial = f"{Var.TUTORIAL_URL}"
  
        msg_text ="""<i><u>𝗬𝗼𝘂𝗿 𝗟𝗶𝗻𝗸 𝗚𝗲𝗻𝗲𝗿𝗮𝘁𝗲𝗱 !</u></i>\n\n<b>📂 Fɪʟᴇ ɴᴀᴍᴇ :</b> <i>{}</i>\n\n<b>📦 Fɪʟᴇ ꜱɪᴢᴇ :</b> <i>{}</i>\n\n<b>📥 Dᴏᴡɴʟᴏᴀᴅ :</b> <i>{}</i>\n\n<b> 🖥WATCH  :</b> <i>{}</i>\n\n<b>🚸 Nᴏᴛᴇ : LINK WILL NOT EXPIRE UNTIL I DELETE</b>""" 
  
        await log_msg.reply_text(text=f"{online_link}", disable_web_page_preview=True,  quote=True) 
        await m.reply_text( 
            text=msg_text.format(get_name(log_msg), humanbytes(get_media_file_size(m)), online_link, online_link), 
            quote=True, 
            disable_web_page_preview=True, 
...
async def channel_receive_handler(bot, broadcast): 
    ...
    try: 
        ...
        await bot.edit_message_reply_markup( 
            ...
            reply_markup=InlineKeyboardMarkup( 
                [ 
                    [InlineKeyboardButton("🖥STREAM ", url=online_link), 
